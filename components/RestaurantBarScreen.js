...
export default function RestaurantBarScreen({ route, navigation }) {
    ...
    const restaurantTotal = items.map(item => item.amounts).reduce((prevTotal, value) => prevTotal + value, 0).toFixed(2)
    ...
    useEffect(() => {
        db.transaction(tx => {
            tx.executeSql('create table if not exists restaurant (id integer primary key not null, amounts int, title text, entrydate text);');
        }, null, updateList);
    }, []);

    const saveItem = () => {
        db.transaction(tx => {
            tx.executeSql('insert into restaurant (amounts, title, entrydate) values (?, ?, ?);', [parseFloat(amount).toFixed(2), title, entrydate]);
        }, null, updateList
        )
    }
    ...
    const updateList = () => {
        db.transaction(tx => {
            tx.executeSql('select * from restaurant;', [], (_, { rows }) =>
    ...
    return (
        <View style={styles.categoryStyles.container}>
            ...
            {/* <View style={{ alignItems: 'center' }}>
                <Text style={styles.categoryStyles.total}>Total: {restaurantTotal}€ </Text>
                <View style={{ alignItems: 'center' }}>
