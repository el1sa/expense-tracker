...
export default function TransportationScreen({ route, navigation }) {
    ...
    const transportationTotal = items.map(item => item.amounts).reduce((prevTotal, value) => prevTotal + value, 0).toFixed(2)
    ...
    useEffect(() => {
        db.transaction(tx => {
            tx.executeSql('create table if not exists transportation (id integer primary key not null, amounts int, title text, entrydate text);');
        }, null, updateList);
    }, []);

    const saveItem = () => {
        db.transaction(tx => {
            tx.executeSql('insert into transportation (amounts, title, entrydate) values (?, ?, ?);', [parseFloat(amount).toFixed(2), title, entrydate]);
        }, null, updateList
        )
    }
    ...
    const updateList = () => {
        db.transaction(tx => {
            tx.executeSql('select * from transportation;', [], (_, { rows }) =>
    ...
    const deleteItem = (id) => {
        db.transaction(tx => {
            tx.executeSql(`delete from transportation where id = ?;`, [id]);
        }, null, updateList
        )
    }
    ...
    return (
        <View style={styles.categoryStyles.container}>
            ...
            {/* <View style={{ alignItems: 'center' }}>
                <Text style={styles.categoryStyles.total}>Total: {transportationTotal}€ </Text>
                <View style={{ alignItems: 'center' }}>
