...
export default function GroceryScreen({ route, navigation }) {
    ...
    const healthTotal = items.map(item => item.amounts).reduce((prevTotal, value) => prevTotal + value, 0).toFixed(2)
    ...
    useEffect(() => {
        db.transaction(tx => {
            tx.executeSql('create table if not exists health (id integer primary key not null, amounts int, title text, entrydate text);');
        }, null, updateList);
    }, []);

    const saveItem = () => {
        db.transaction(tx => {
            tx.executeSql('insert into health (amounts, title, entrydate) values (?, ?, ?);', [parseFloat(amount).toFixed(2), title, entrydate]);
        }, null, updateList
        )
    }
    ...
    const updateList = () => {
        db.transaction(tx => {
            tx.executeSql('select * from health;', [], (_, { rows }) =>
    ...
    const deleteItem = (id) => {
        db.transaction(tx => {
            tx.executeSql(`delete from health where id = ?;`, [id]);
        }, null, updateList
        )
    }
    ...
    return (
        <View style={styles.categoryStyles.container}>
            ...
            {/* <View style={{ alignItems: 'center' }}>
                <Text style={styles.categoryStyles.total}>Total: {healthTotal}€ </Text>
                <View style={{ alignItems: 'center' }}>
